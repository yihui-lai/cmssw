#ifndef GeneratorInterface_Pythia8Interface_PlutoPiPiDilepton_h
#define GeneratorInterface_Pythia8Interface_PlutoPiPiDilepton_h
...
namespace gen {
  namespace pluto {
    ...
    std::array<TLorentzVector, 4> pipiMagneticPointlike(const TLorentzVector& parent,
      ...
      const double bound = std::pow(lambda(smin / M2, qmin / M2), 1.5) * (smax / M2) * std::pow(1 - smin / smax, 1.5) *
                            std::sqrt(1 - qmin / qmax);
      if (!(bound > 0) || !std::isfinite(bound))
        throw std::runtime_error("Invalid PiPi envelope");
      for (unsigned attempt = 0; attempt < 1000000; ++attempt) {
        ...
        // Output ordering is l-, l+, pi+, pi- as used by the decay handler.
