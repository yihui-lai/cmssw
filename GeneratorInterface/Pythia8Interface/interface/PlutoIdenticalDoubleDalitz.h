#ifndef GeneratorInterface_Pythia8Interface_PlutoIdenticalDoubleDalitz_h
#define GeneratorInterface_Pythia8Interface_PlutoIdenticalDoubleDalitz_h
  ...
  namespace pluto {
    namespace fourlepton {
      ...
      inline Complex contraction(const Current& a, const Current& b, const TVector3& q) {
        return q.X() * (a[1] * b[2] - a[2] * b[1]) + q.Y() * (a[2] * b[0] - a[0] * b[2]) +
               q.Z() * (a[0] * b[1] - a[1] * b[0]);
      }

      // Amplitude-level form factor for dressing each virtual-photon
      // propagator in the direct/exchange diagrams below. Matches the
      // |F|^2 already used elsewhere at the amplitude level:
      // |ffAmplitude(q2)|^2 == that same weight. rhoMass<=0 disables it
      // (F=1), preserving the exact original constant-form-factor behavior
      // for existing callers/tests. useEtaPade=true (eta/221 only) instead
      // uses the data-fitted eta TFF (PlutoEtaTFF.h) -- real-valued, since
      // eta never reaches the resonance region where an imaginary part
      // would appear.
      inline Complex ffAmplitude(double q2, double rhoMass, double rhoGamma, bool useEtaPade = false) {
        if (useEtaPade)
          return Complex(etaPadeFormFactorNorm(q2), 0.);
        if (!(rhoMass > 0))
          return Complex(1., 0.);
        return (rhoMass * rhoGamma) / Complex(rhoMass * rhoMass - q2, -rhoMass * rhoGamma);
      }
      ...
      public:
        Amplitudes(
            std::array<TLorentzVector, 4> p, double m, double rhoMass = 0, double rhoGamma = 0, bool useEtaPade = false) {
          TLorentzVector parent;
          for (const auto& daughter : p)
            parent += daughter;
          const double M = parent.M();
          if (!(M > 4 * m && m > 0))
            throw std::runtime_error("Invalid four-lepton masses");
          for (auto& daughter : p)
            daughter.Boost(-parent.BoostVector());
          const auto qD = p[0] + p[1], rD = p[2] + p[3];
          const auto qX = p[0] + p[3], rX = p[2] + p[1];
          // Each diagram's two virtual photons are dressed independently, at
          // their own (generally different, direct vs. exchange) invariant
          // masses -- this is why the fix has to happen here, at the
          // amplitude level, not as a single overall probability reweight.
          const Complex ffD = ffAmplitude(qD.M2(), rhoMass, rhoGamma, useEtaPade) *
                              ffAmplitude(rD.M2(), rhoMass, rhoGamma, useEtaPade);
          const Complex ffX = ffAmplitude(qX.M2(), rhoMass, rhoGamma, useEtaPade) *
                              ffAmplitude(rX.M2(), rhoMass, rhoGamma, useEtaPade);
          dScale_ = (M / (qD.M2() * rD.M2())) * ffD;
          xScale_ = (M / (qX.M2() * rX.M2())) * ffX;
          dAxis_ = qD.Vect();
          xAxis_ = qX.Vect();
      ...
      private:
        ...
        Complex dScale_, xScale_;
      };

      struct Scores {
        double direct = 0, exchanged = 0, coherent = 0;
      };
      inline Scores scores(const std::array<TLorentzVector, 4>& p,
                           double m,
                           double rhoMass = 0,
                           double rhoGamma = 0,
                           bool useEtaPade = false) {
        const Amplitudes amplitudes(p, m, rhoMass, rhoGamma, useEtaPade);
        Scores result;
          ...
          for (unsigned b = 0; b < 2; ++b)
            for (unsigned c = 0; c < 2; ++c)
              for (unsigned d = 0; d < 2; ++d) {
                ...
                result.direct += std::norm(values[0]);
                result.exchanged += std::norm(values[1]);
    ...
    // coherent rejection. |D-X|^2 <= 2(|D|^2+|X|^2) holds for *any* complex
    // D, X (expand |D-X|^2 = |D|^2+|X|^2-2Re(D X*) <= |D|^2+|X|^2+2|D||X| <=
    // 2(|D|^2+|X|^2) by AM-GM) -- so this bound survives the rho0-pole
    // dressing in Amplitudes/ffAmplitude unchanged; no new envelope
    // derivation was needed to add it. rhoMass<=0 (the default) reproduces
    // the original constant-form-factor behavior exactly. Identical-particle
    // factorials affect rates, not this normalized exclusive shape.
    template <class Flat>
    std::array<TLorentzVector, 4> identicalPointlike(const TLorentzVector& parent,
                                                      double m,
                                                      Flat flat,
                                                      double rhoMass = 0,
                                                      double rhoGamma = 0,
      ...
      for (unsigned attempt = 0; attempt < 100000; ++attempt) {
        ...
        const auto score = fourlepton::scores(out, m, rhoMass, rhoGamma, useEtaPade);
